...
import {
  Card, Col, Image, Row, Space,
} from 'antd';
import { Image as ImageType } from '../../types/imageType';

export interface IProductImagesGrid {
  images: ImageType[];
}
...
function ProductImagesGrid(props: IProductImagesGrid): JSX.Element {
  const { images } = props;
    ...
    <Col xxl={12} xl={12} lg={12} md={12} sm={22}>
      ...
      >
        ...
        >
          ...
          <Row gutter={12}>
            {
              imageOrders.map((imageOrder) => (
                <Col span={8} key={imageOrder}>
                  <Image
                    width="100%"
                    className="rounded-4"
                    src={images[imageOrder]?.secureUrl || ''}
                    fallback={fallback}
                  />
                </Col>
              ))
